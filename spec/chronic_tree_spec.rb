...
describe ChronicTree do
  ...
  context "Tree Traversal" do
    ...
    it "s: access children" do
      ...
      expect(@lv2_child_org.children.size).to be == 0
    end
    ...
    it "s: access descendants" do
      ...
      expect(@root_org.flat_descendants.second.id).to be == @lv2_child_org.id
      expect(@lv1_child_org.descendants.size).to be == 1
      expect(@lv1_child_org.flat_descendants.size).to be == 1
      expect(@lv1_child_org.flat_descendants.first.id).to be == @lv2_child_org.id
      expect(@lv2_child_org.descendants.size).to be == 0
    end
    ...
    context "Tree isn't empty" do
      ...
      context "Self is in the tree" do
        ...
        it "s: add children" do
          ...
          expect(@root_org.descendants.second.size).to be == 2
        end
        ...
        it "s: remove descendants" do
          ...
          expect(@lv2_child_org.existed?).to be_falsy
          expect(@root_org.descendants.size).to be == 1
          expect(@root_org.flat_descendants.size).to be == 1
        end

        it "s: change parent to another object" do
          @lv2_child_org.change_parent(@root_org)
          expect(@lv1_child_org.children.size).to be == 0
